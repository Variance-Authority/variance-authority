...
| [Storage and cache primitives](0011-storage-and-cache-primitives.md) | Applicability pruning, component hashes, nine inspection rules, and three baseline backends — every half an operator would want to choose between. | **The seams.** The render cache is welded to the baseline store despite the two having opposite loss semantics; the layout is private to one backend; the rule set is a closed union; nothing reduces a render document before hashing it. Five items, ordered, in the file. |
