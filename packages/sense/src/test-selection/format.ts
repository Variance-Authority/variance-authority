...
function dictionary(coverage: TestCoverage): readonly string[] {
  ...
  for (const module of coverage.modules) {
    ...
    for (const block of module.blocks) {
      ...
      // Not the crossings. A crossing names a test file, every one of which the
      // walk over `tests` above already added, and one that names a file no test
      // observed is refused by the encode a few lines below. So a loop over them
      // here could only ever re-add a string the set already held, or add one on
      // an input about to throw — a read per crossing, and a snapshot has an
      // order of magnitude more crossings than it has distinct strings.
    }
  }
