...
| B12 | **history** — what accumulates across runs, and where it lives | **wired, and answering one of its five questions** ([what is left](../specs/0002-history-store.md)). The contract gap that blocked it is decided — `current()`, the one read about the present, which never truncates because a missing previous row is a change that did not happen (ADR-0031). `variance run` now records its run, the hashes that moved, and every subject that failed to read the same way twice, then reports how often that has happened before, dividing by the sweeps that could have seen it (ADR-0032). Still open: no run resolves a token, so the spec's own headline case is unreachable; `churn` and `reach` have rows and no caller; and acceptance is never recorded, so a drift total today would be zero. First implementation refuted and retired ([epitaphs](epitaphs.md)) |
...
- **The documentation is checked now, and one class of it still is not.**
  ...
  path and every `file:line` reference across all 104 markdown files, and compiles
