...
| function entry | 2,798 | entered, and owns every statement before the first decision |
| branch outcome | 4,242 | `if`/`else`, including the **synthesized** `else` of a bare `if` |
| continuation | 2,344 | the region *after* a decision, up to the next one |
| `await` resume | 556 | execution came back — the stack after is not the stack before |
| loop body | 629 | the body was entered at least once |
...
other two columns — 15,056 statements collapse to 2,344 continuations, because a
