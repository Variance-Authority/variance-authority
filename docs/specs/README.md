...
|---|---|---|
| [History service and drift queries](0002-history-store.md) | The rows, the drift arithmetic, both backends, the wire — and since 2026-08-10 a caller: `variance run` records its run, the hashes that moved, and every subject that failed to read the same way twice, then reports how often that has happened before ([ADR-0031](../context/adr/0031-the-run-asks-what-is-recorded-now.md), [ADR-0032](../context/adr/0032-a-flake-rate-divides-by-the-runs-that-asked.md)). | **A caller for the questions, and the journey run end to end.** `churn`, `valueJourney` and `reach` have rows to read — correct ones, since acceptance became a row of its own and tokens acquired a source — and nothing asks them. The spec's headline case, a button gaining 2px eleven times, is reachable and has never been produced: that needs eleven runs and eleven approvals, not more code. |
