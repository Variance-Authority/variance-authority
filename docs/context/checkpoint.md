...
| B28 | **presentation sensing** — rendered relationships exposed to a coding agent without a design policy or regression lifecycle | landed as [`@variance-authority/presentation`](../../packages/presentation) ([`docs/presentation.md`](../presentation.md), ADR-0050/0051/0052). One live Playwright subject becomes a graph of semantic anchors, geometry, spacing, axes, inferred baselines, surfaces, prominence, repetition, and calibrated relationship findings, with diagnostic paint from the same report. Findings and paint identify the structural owner of their relationship. A pure focus keeps nested boxes separate by default, isolates a stable finding id, and paints without another acquisition; explicit alignment and spacing readings cover product-known flows across wrappers and the rhythm between heterogeneous siblings without declaring all descendants peers. Repeated structures also retain the spacing relation from a distinct leading label into their body separately from the relation among body peers; a collision paints both steps. Acquisition follows that boundary too: unrelated document images do not hold the subject open and React provenance the report does not consume is not collected. A caller-owned static archive can explicitly replace stabilization with an empty recipe when its page clock cannot advance. Playwright's ARIA snapshot remains an independent retained signal: empty and partial are readings, absence alone is unobserved. Density, height, margins, utilization, and content volume are telemetry and cannot create findings. The package owns no baseline, approval, build verdict, or regression classification; an optional report comparison keeps content identity and information counts beside finding counts. The packaged [`variance-presentation` skill](../../packages/presentation/skills/variance-presentation/SKILL.md) makes hierarchy mapping the first move before findings or paint. |
...
| M37 | B28 | Sense and paint presentation relationships on one live subject while retaining independent ARIA evidence and refusing density policy | **mixed, and four corrections were architectural** — the original ten pure and two Chromium cases established local relationship sensing, then live use showed that a locator is not a peer set. Owner focus now keeps nested evidence out of a composition, explicit flow alignment distinguishes a 2px centre spread on Atlassian navigation from zero spread on the GitHub and Paper controls, explicit composition spacing retains relationships between heterogeneous sibling regions that repetition inference cannot group, and repeated structures compare their leading-label relation with their body-peer relation rather than hiding both inside one internal median. On local `#agents`, seven acquisitions had a 76.63ms median; 1,000 pure owner projections took 8.07ms, and one finding reduced 189 paint instructions to one. A content-preserving five-record edit moved eight findings to zero. The supplied Underwriter archive measured five consecutive major-region gaps at 0–0.01px before a 38.38px transition; its 31 demand rows were separated at 11.18–11.19px, while five inferred patterns now report the pictured 4px leading-to-body and 4px body-to-body hierarchy collision. |
