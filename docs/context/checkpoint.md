...
| B28 | **presentation sensing** — rendered relationships exposed to a coding agent without a design policy or regression lifecycle | landed as [`@variance-authority/presentation`](../../packages/presentation) ([`docs/presentation.md`](../presentation.md), ADR-0050). One live Playwright subject becomes a graph of semantic anchors, geometry, spacing, axes, inferred baselines, surfaces, prominence, repetition, and calibrated relationship findings, with diagnostic paint from the same report. Playwright's ARIA snapshot remains an independent retained signal: empty and partial are readings, absence alone is unobserved. Density, height, margins, utilization, and content volume are telemetry and cannot create findings. The package owns no baseline, approval, build verdict, or regression classification; an optional report comparison keeps content identity and information counts beside finding counts. The packaged [`variance-presentation` skill](../../packages/presentation/skills/variance-presentation/SKILL.md) carries the live, raw-capture, headed, ARIA, paint, and re-sensing operating contract for coding agents, including the current stabilization and browser-lifetime limits. |
...
| M37 | B28 | Sense and paint presentation relationships on one live subject while retaining independent ARIA evidence and refusing density policy | **mixed, and the correction was architectural** — ten pure cases separated dense coherence, large margins, collapsed repetition, state-explained variance, surfaces, alignment, baselines, absent layout, separately sensitive ARIA readings, and content identity from equal counts. Two Chromium cases then retained ARIA, found and painted a 31-record collapse, and left a 256px margin and dense typography as telemetry. The first browser entry was placed in the visual-regression Playwright package and rejected: the offering supports a live edit and owns no baseline or verdict, so acquisition and paint moved into the presentation package itself. The supplied Underwriter benchmark had no page artifact or URL, so that named interface remains unmeasured. |
...
**B28 is landed as a sensing and support surface, not another regression mode.**
Next: run `sensePresentation` against the named Underwriter page when its artifact
or reachable URL is available, and let that external design challenge the local
relationship inference and calibration.
