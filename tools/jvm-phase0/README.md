...
- `presence/` is our own recorder, a Java agent that replaces JaCoCo. It sets one flag per method on entry, from a fixed `boolean[]` on the boot class path, and writes no branch or line probes. A recorded method carries every line it holds, so a line-grain selection only widens. With the listener beside it, each test class becomes one `record.jsonl` row directly, with no exec file and no analyze step. Synthetic methods other than lambda bodies are not probed. A class is named by the one file that exists as `<root>/<package>/<SourceFile>` under the `sources` roots (default `src/main/java:src/test/java:src/main/kotlin:src/test/kotlin`), or else by the one file of that name under them. A class that names no file or several is `unknown` in the row, and a class that fails to instrument is listed in every later row; `score.mjs` runs a test whose row lists either. A class without a `SourceFile` attribute, such as a proxy or a class a test defines at runtime, is not probed. On Commons Lang a full per-class record adds 1.5% to the test phase, where JaCoCo with the dump adds 5.6% before analysis.
  ...
  A service that a test runner calls writes parts instead, the frames sense's fold joins to the runner's cases by journey. Hand the JVM a parts directory, as `-Dva.parts=<dir>` or the `VARIANCE_AUTHORITY_PARTS` variable the runner already exports, and on exit the agent's `Parts` converts its record into two files there. `jvm-<pid>-<uuid>.vac` holds one frame per journey, plus one frame with no journey for the between windows, which the fold charges to every case that crossed into this process. `jvm-<pid>-<uuid>.rec` holds the regions those frames name, cut as `Coverage` cuts them, under the recipe `jvm:presence/methods-v1`. Files are named relative to the nearest directory above the working directory that holds `.git`, so the names match the runner's. Without `va.out`, the record goes to a directory of the process's own, so two services never convert each other's rows. A driver's table isn't needed: the case that called is the one that minted the journey. `java -cp va-presence.jar va.presence.Parts <record.jsonl> <dir>` converts a record already on disk. A class that resolved to no source file is left out of the parts.
...
- `parts.mjs` proves the same crossing from a Jest run, with nothing in between but HTTP. The shop's `jest/` cases call it with `fetch` and put `journeyCookie()` on the request themselves. The harness runs the shop under the agent with a parts directory, runs Jest in band against it, stops the JVM so it writes its parts, and only then finalizes Jest's journey file. It asserts that a method one case's request ran is charged to that case alone, that the readiness probe and the JVM's startup are charged to both cases that crossed, that a case which never called the shop is absent, and that a change to a Java line selects the Jest file that reached it. It runs in band because journeys that are open at the same time share their windows.
...
node tools/jvm-phase0/parts.mjs "$WORK/parts" "$WORK/bin"
```
