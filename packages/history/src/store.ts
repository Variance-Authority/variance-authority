...
export interface HistoryStore {
   ...
   *
   * TODO: give `lastChanged` a caller. It is implemented in every backend and
   * served by `packages/server/src/http.ts` and
   * `packages/tribunal/src/worker.ts`; it closes when a surface deliberately
   * asks when a component last moved.
   */
