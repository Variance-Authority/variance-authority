...
- **The documentation is checked now, and one class of it still is not.**
  ...
  path and every `file:line` reference across all 200 markdown files, and compiles
...
**B26 is landed, including the whole-file and MCP queries.** Next: expose both
point and whole-file queries directly over the compact execution view when a
collector supplies its persisted path trie.
