...
|---|---|---|
| [History service and drift queries](0002-history-store.md) | The rows, the drift arithmetic, both backends, the wire — and since 2026-08-10 a caller: `variance run` records its run, the hashes that moved, and every subject that failed to read the same way twice, then reports how often that has happened before ([ADR-0031](../context/adr/0031-the-run-asks-what-is-recorded-now.md), [ADR-0032](../context/adr/0032-a-flake-rate-divides-by-the-runs-that-asked.md)). | **`reach`, and the journey run end to end.** Four of the five questions are asked by the run and answered in the report; `reach` asks about the suite rather than about anything a run observed, and waits for a surface shaped for that. The spec's headline case, a button gaining 2px eleven times, is now produced by the pipeline and has never been *run*: that needs eleven runs and eleven approvals, not more code. |
