...
| [Hosted: who the caller is, and what the bill counts](0014-hosted-who-the-caller-is-and-what-the-bill-counts.md) | Two capability tokens in `tribunal` with a constant-time comparison and one indistinguishable refusal; a history service that will not start without a token; a remote baseline store that takes a bearer. | **An identity, a tenant and a meter.** A token resolves to a capability and never to *who* or *whose*, so one deployment is one project and an approval names nobody. Nothing counts anything, and a counter would be a number the customer cannot check. `serveRenderer` has no authentication at all. The one item that must land before the first deployment rather than after is in the file. |
| [A real agent](0013-a-real-agent.md) | Five MCP tools as pure functions over a report, a protocol codec, `variance serve` — tested against text. `Intent` and `adjudicate` in `core`, reachable from no boundary an agent meets. | **Evidence, and both loops.** No agent has ever called any of it. The fixer loop lacks its existence proof and its two-arm measurement; the author loop lacks a subject-scoped run, the intent wire, and the stated boundary that an agent proposes and never promotes. |
