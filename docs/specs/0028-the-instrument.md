...
file is added. The table below is one such reading, at 346 product files.

| Probe | Count | What it means |
|---|---|---|
| module | 346 | the module's top level evaluated |
| function entry | 2,797 | entered, and owns every statement before the first decision |
| branch outcome | 4,236 | `if`/`else`, including the **synthesized** `else` of a bare `if` |
| continuation | 2,342 | the region *after* a decision, up to the next one |
| `await` resume | 556 | execution came back — the stack after is not the stack before |
| loop body | 630 | the body was entered at least once |
...
other two columns — 15,065 statements collapse to 2,342 continuations, because a
