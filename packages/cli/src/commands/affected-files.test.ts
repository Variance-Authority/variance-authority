...
describe('every file a diff affects', () => {
  it('holds the changed file itself, and every file that imports it', () => {
    const affected = affectedFiles(GRAPH, ['src/lib/parse.py'], ROOTS);
    if (refused(affected)) throw new Error(affected.whole);

    expect(affected.files).toEqual(['src/app/main.py', 'src/app/other.py', 'src/lib/parse.py']);
  ...
  it('answers a file nothing imports with that file, and not with nothing', () => {
    ...
    // width of the change.
    const affected = affectedFiles(GRAPH, ['src/lib/alone.py'], ROOTS);
    if (refused(affected)) throw new Error(affected.whole);

    expect(affected.files).toEqual(['src/lib/alone.py']);
  });

  it('does not walk an edge that is erased before anything runs', () => {
    ...
    expect(affected.files).not.toContain('src/app/typed.py');
  });
  ...
  it('refuses a changed file under the scanned roots the graph does not hold', () => {
    ...
    const affected = affectedFiles(GRAPH, ['src/lib/parse.py', 'src/lib/new.py'], ROOTS);
    expect(refused(affected) && affected.unscanned).toEqual(['src/lib/new.py']);
  });

  it('refuses a diff no part of which is in the graph', () => {
    const affected = affectedFiles(GRAPH, ['yarn.lock', 'Dockerfile'], ROOTS);
