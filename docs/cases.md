...
| --- | --- | --- |
| Where is the UI already ready? | Storybook, served route/static output, Playwright Test, browserless unit DOM, custom host | Selects lifecycle, discovery, and naming adapter |
...
- state can be reached by deterministic navigation/setup;
...
- DOM/resource disclosure across that renderer boundary is acceptable.
...
must not own a browser. capture writes semantic/document evidence, writeCapture
persists it, and a later CLI process loads it through captureCollector and
renders locally or remotely.

Choose it when:

- ordinary Jest/Vitest lifecycle must remain unchanged;
- browser startup does not belong in unit workers;
- resource bytes can be closed during capture;
- a two-process acquisition/render workflow is acceptable.
...
vendor-operated review surface, browser/device fleet, support contract, branch
baseline workflow, perceptual differ, or compliance commitment. Self-operation
