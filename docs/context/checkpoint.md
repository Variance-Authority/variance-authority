...
| B29 | **update attention** — distinguish the component instance that initiated a React commit from everything React rendered because of it | landed across `react`, Eyes and MCP ([ADR-0055](adr/0055-update-initiators-are-structural-attention.md), [journal 0039](journal/0039-the-updater-had-a-location.md)). The pre-navigation Eyes agent installs the commit tap before application code, copies `memoizedUpdaters` as bounded innermost-first paths of name, key and props digest, and places each commit in the authored AAA chronology. `variance_testing_surface` separates initiators structurally inside an addressed target from those outside it, beside whole-test execution crossings rather than collapsing the three. `PerformedWork` remains the rendered-component reading. Public bounded Fiber subtree, parent-chain, composite-path and source-location helpers expose the same diagnostic seam without a page registry or mutation channel. The repo skill routes an agent through the inventory, Vantage, attention and source evidence while preserving unavailable versus measured empty. |
