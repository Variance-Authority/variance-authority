...
| [A contract is a subject](0031-a-contract-is-a-subject.md) | Nothing. Every subject is a rendered document or an image of one: `SubjectRef.kind` is `story`, `route` or `fixture`, and every band in `BANDS` is a fact about markup, boxes or pixels. | **The material, the bands, and the direction rule.** An OpenAPI description, a GraphQL schema, a route table or a JSON body cannot enter the run, so a removed response field is watched by a text diff or by nothing. The parse is what a diff lacks: the same edit — an enum gaining a member, a field becoming nullable — is safe in one position and breaking in the other, and only a document that knows request from response can say which. |
