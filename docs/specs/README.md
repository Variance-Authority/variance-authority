...
| [A contract is a subject](0031-a-contract-is-a-subject.md) | Nothing. Every subject is a rendered document or an image of one: `SubjectRef.kind` is `story`, `route` or `fixture`, and the collector boundary promises a `RenderDocument` or a sentence saying why there is none. | **The material, the identity, and the second join.** An OpenAPI description, a GraphQL schema, a route table or a plain JSON value cannot enter a run, so each is watched by a detector that starts from nothing every time it runs. The detectors are good and mostly free; what none of them keeps is a record — an approval that survives the next commit, a count of how often this shape has come back, and one docket covering the component that moved and the response that changed with it. |
