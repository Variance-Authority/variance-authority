...
- `listener/` is a JUnit Platform listener, loaded as a Java agent next to JaCoCo, that dumps and resets the JaCoCo store at each test class boundary.
...
- `record-maven.sh` runs the suite once, as `bare`, `agent` (probes only) or `split` (probes plus the per-class dump).
