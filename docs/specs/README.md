...
| [Storage and cache primitives](0011-storage-and-cache-primitives.md) | Applicability pruning, component hashes, nine inspection rules, three baseline backends, and — since 2026-08-04 — the cheap settlement path, which reads no baseline image at all when nothing moved. | **The seams.** The render cache is welded to the baseline store despite the two having opposite loss semantics; the layout is private to one backend; the rule set is a closed union; nothing reduces a render document before hashing it. Item 0 of five is discharged; the remaining four are in the file, ordered. |
