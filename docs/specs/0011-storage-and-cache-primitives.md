...
|---|---|---|
| Clean the HTML, match the CSS to what survives | CSS applicability pruning ships in `@variance-authority/dom`: 1010 rules to 1 on the accretion fixture in `packages/dom/src/collect.test.ts` (ADR-0003). Structural aliasing ships for the *semantic* snapshot. | Nothing reduces the **render** document. It keeps ids and classes deliberately, and its digest churns on both. |
...
collector (the contract SubjectSource `packages/cli/src/commands/collector.ts:270`),
...
- **Losing a baseline is fatal.** `null` means "looked, nothing there", which
  ...
  `RasterStoreError packages/raster/src/store.ts:54` and the `REFUSAL` beside it.
