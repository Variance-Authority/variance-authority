...
> *"Two modes: durable and temporal data. durable: all images should be rendered
> in the same 'machine' to be equal. Then will be saved. ephemeral: rendered
> here, compared with here, later discarded."*

## Context

ADR-0010 established that the *semantic* representation is portable — machine-bound
inputs cannot reach the box tree — and confined the container to the raster
residue. It did not say how that residue is retained, and the answer is not one
thing.
...
But a comparison whose two sides are produced in one run has no such requirement.
...
- **Ephemeral.** Both images are rendered now, by one renderer, and discarded.
  ...
  stored artifact, no comparability question. This mode *removes* the requirement
  rather than satisfying it.
- **Durable.** One image is rendered now and compared against a stored one. The
  store is **partitioned by renderer identity** — `<root>/<identityDigest>/…` — so
  a baseline written by one machine is not merely rejected by another, it is not
  in the directory the other machine reads.
...
`changed`.** `find` deliberately scans sibling identities rather than only its
own, so the answer is *"we have seen this, on a machine you are not"* rather than
*"new subject"* — a wrong-machine run becomes one sentence instead of a mass
...
## The render cache falls out of this
...
- Sharing a raster baseline across machines. Pixels are machine-bound; this ADR
  confines the cost to the artifact that has it and does not pretend to remove it.
...
- A default that stores images. The cheap mode is the one that keeps nothing, and
  it should stay the one that requires no configuration.
...
alarm rather than a false `unchanged`, which is the safe direction, and it is the
same hole ADR-0010 left open under "one machine".
