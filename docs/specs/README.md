...
| [Order dependence in a run](0012-order-dependence-in-a-run.md) | The corner-cut, taken: one page across N captures. And the detector — `@variance-authority/session`, 1,060 lines of probe, ledger, reads-derivation and agent-ready report. | **The wire between them.** No manifest depends on `session`, because it is a session *runner* rather than an instrumentation layer: adopting it means rewriting the collector it would protect. So a run neither rinses nor detects. Closing it needs one optional method on `Collector` and a budgeted second pass. |
