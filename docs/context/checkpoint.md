...
| B26 | **source-to-test lookup** — an editor asks which named tests reached one function, line, or whole source file, and how far away they were | landed as the runner-independent `coveringTests` and `coveringTestsInFile` APIs. A source line resolves to the innermost indexed execution region, a function name resolves to its function-entry region, and the result contains individual test identities ordered by their shortest observed call-stack depth. The file query partitions every indexed source line into inclusive ranges with one shared answer, including empty answers for indexed regions no test reached while omitting lines the index cannot describe. Stable ids distinguish duplicate names. The API is a pure reverse lookup over an `ExecutionIndex` a collector or IDE supplies; it does not add test-case selection, collection hooks, persistence, or an editor protocol. |
...
**B26 is landed, including the whole-file query.** Next: expose both point and
whole-file queries directly over the compact execution view when a collector
supplies its persisted path trie.
