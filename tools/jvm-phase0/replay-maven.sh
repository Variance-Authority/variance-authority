#!/bin/sh
# Records a split run and its method map at each of the last N first-parent commits.
# Usage: replay-maven.sh <work dir> <repo dir> <name> <N> <includes> <source root> <classes dir>
# Output: <work dir>/replay/<name>/<sha>/{events.tsv,record.jsonl,status.tsv,diff.patch} and order.txt (oldest first).
set -eu
HERE=$(cd "$(dirname "$0")" && pwd)
WORK=$1 REPO=$2 NAME=$3 N=$4 INCLUDES=$5 SRC=$6 CLASSES=$7
...
for sha in $(cat "$BASE/order.txt"); do
  ...
  sh "$HERE/record-maven.sh" "$WORK" "$REPO" split "$dir/exec" "$INCLUDES" > "$dir/run.txt" 2>&1
