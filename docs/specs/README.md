...
| [Cause-first ranking on every path](0017-cause-first-ranking-on-every-path.md) | The ranking, per-component hashing, and the sidecar that settles an unchanged run without decoding a PNG. | **The previous revision's document, at comparison time.** A stored baseline is an image, so a run orders by area — measured as backwards by 6×. Carry the document in the sidecar or fetch it from the cache; the two shapes are in the file. Decide before building. |
