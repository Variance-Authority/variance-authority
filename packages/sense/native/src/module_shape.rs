...
//! created, and nothing in it runs until something calls.
...
pub fn parse<'a>(allocator: &'a Allocator, file: &str, text: &'a str, bodies: bool) -> Option<Program<'a>> {
    ...
    Erase { allocator, bodies }.visit_program(&mut program);
    Some(program)
}
...
/// A class whose definition runs nothing: no base expression, no decorator, no
/// static field or block, no computed key. Its instance fields run when it is
/// constructed, which is a value the class carries rather than a load.
pub fn plain_class(class: &Class) -> bool {
    class.heritage.is_none()
        ...
        && class.body.body.iter().all(|member| match member {
            ...
            ClassElement::MethodDefinition(it) => !it.computed && it.decorators.is_empty(),
