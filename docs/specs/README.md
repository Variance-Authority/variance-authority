...
| [A test is selected by what it executed](0027-a-test-is-selected-by-what-it-executed.md) | The product definition, and the measurements that argue for it: a probe census over this repository's own source, and probe cost on realistic work against V8 precise coverage. | **The whole capability**, split into [0028](0028-the-instrument.md), [0029](0029-what-a-run-remembers.md) and [0030](0030-a-diff-lands-on-blocks.md). `variance` cannot run a test suite. The static selector answers *37 files import something connected to this*; the number worth reporting is *7 tests executed it, 2 of them directly*, and the hole it closes is the one [`selecting.md`](../selecting.md) names itself — a component imported but never rendered. |
| [The instrument](0028-the-instrument.md) | Nothing. A measured probe set — 9,961 probes over 301 product files, 0.355× Istanbul's statement sites — and a cost of 1.05× on realistic work with the call stack maintained. | **The transform, the probes, the worker runtime and the runner seams.** No source is instrumented. This is the only part that manufactures an artifact rather than consuming one, and it carries the kill criterion: above 1.35× overhead the project is not worth building, and nothing else can measure that. |
