...
| M37 | B28 | Sense and paint presentation relationships on one live subject while retaining independent ARIA evidence and refusing density policy | **mixed, and two corrections were architectural** — the original ten pure and two Chromium cases established local relationship sensing, then live use showed that a locator is not a peer set. Owner focus now keeps nested evidence out of a composition, and explicit flow alignment distinguishes a 2px centre spread on Atlassian navigation from zero spread on the GitHub and Paper controls supplied as positive cases. On local `#agents`, seven acquisitions had a 76.63ms median; 1,000 pure owner projections took 8.07ms, and one finding reduced 189 paint instructions to one. A content-preserving five-record edit moved eight findings to zero. The supplied Underwriter benchmark still has no page artifact or URL, so that named interface remains unmeasured. |
