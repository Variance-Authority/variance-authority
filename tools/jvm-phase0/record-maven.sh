...
# Usage: record-maven.sh <work dir> <repo dir> <mode: bare|agent|split> <exec out dir> [includes]
...
set -eu
WORK=$1 REPO=$2 MODE=$3 OUT=$4 INCLUDES=${5:-*}
mkdir -p "$OUT"
AGENT=""
case "$MODE" in
  bare) ;;
...
docker run --rm -v "$WORK/m2:/root/.m2" -v "$WORK:/va:ro" -v "$REPO:/repo" -v "$OUT:/out" -w /repo \
  maven:3.9-eclipse-temurin-21 sh -euc "
    start=\$(date +%s)
    mvn -B -q -o test $SKIPS -DagentArgs='$AGENT' > /out/mvn.log 2>&1 || { echo FAILED; tail -40 /out/mvn.log; }
