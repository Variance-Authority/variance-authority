...
| `event` | nothing; its `collect` entrypoint additionally needs a Node runtime and a writable directory | announcements a running system makes about its own decisions, and the log a test waits on |
