...
| M37 | B28 | Sense and paint presentation relationships on one live subject while retaining independent ARIA evidence and refusing density policy | **mixed, and three corrections were architectural** — the original ten pure and two Chromium cases established local relationship sensing, then live use showed that a locator is not a peer set. Owner focus now keeps nested evidence out of a composition, explicit flow alignment distinguishes a 2px centre spread on Atlassian navigation from zero spread on the GitHub and Paper controls, and explicit composition spacing retains relationships between heterogeneous sibling regions that repetition inference cannot group. On local `#agents`, seven acquisitions had a 76.63ms median; 1,000 pure owner projections took 8.07ms, and one finding reduced 189 paint instructions to one. A content-preserving five-record edit moved eight findings to zero. The supplied Underwriter archive measured five consecutive major-region gaps at 0–0.01px before a 38.38px transition; its 31 demand rows remained consistently separated at 11.18–11.19px. |
