...
|---|---|---|
| [History service and drift queries](0002-history-store.md) | The rows, the drift arithmetic, both backends, the wire — and since 2026-08-10 a caller: `variance run` records its run, the hashes that moved, and every subject that failed to read the same way twice, then reports how often that has happened before ([ADR-0031](../context/adr/0031-the-run-asks-what-is-recorded-now.md), [ADR-0032](../context/adr/0032-a-flake-rate-divides-by-the-runs-that-asked.md)). | **The token axis, and three questions nobody asks.** No run resolves a design token to a value, so `valueJourney` receives an empty list on every write and the spec's own headline case — a button gaining 2px eleven times — is still unreachable. `churn` and `reach` now have rows to read and no caller. And every observation is written unapproved, because nothing joins acceptance back, so a drift total today would be zero. |
