...
| B23 | **change management** — what a diff reaches, read from source rather than from a bundler | landed and measured ([`docs/selecting.md`](../selecting.md), [journal 0025](journal/0025-what-a-second-scan-costs.md), ADR-0038/0039/0040). Selection's expensive row was *a changed file that declares no component runs everything*, which is every token file, every theme, every hook — so the selector surrendered on exactly the diffs it was bought for. `@variance-authority/sense` scans the source into one `FileRecord` per file and `core/relate` folds them into CSR adjacency with the transpose materialized, so *what depends on this* costs what *what this depends on* costs. The direction convention is the whole trick: a component depends on the file that declares it, so one walk against the arrows from a changed file reaches every importer and every component in one pass. A file whose imports could not be read carries the **sentence** saying so and is traversed as though it changed. `nx` and `turbo` enter as **seeds, never a selection** — they see the one edge a specifier scan cannot, a package importing another's built output, and the price of that edge is project granularity. Beside the traversal, `closureOf` hashes the graph the way `bazel` keys an action: cycles condensed by Tarjan so every member carries one digest, unread inputs marked volatile and propagated. **The cost is the claim.** git names every file's content without opening one, a digest names the parse, and a digest plus a layout digest over the path set names the whole record — 3002 ms cold, 236 ms warm, and the one-file edit is inside the noise. That also answers the Rust question against ADR-0004's gate, and the answer is no: parsing was never the expensive half once cached, resolution was, and resolution is now eliminated rather than made faster. Two halves stay open — component-to-component edges ([0025](../specs/0025-component-relations.md)) and somewhere to store a closure digest ([0026](../specs/0026-selection-by-closure-digest.md)) **The graph is no longer JavaScript-only.** Python, Rust, Java, Kotlin and Swift are read by tree-sitter into the same `FileRecord` space, `oxc` keeps JavaScript and TypeScript on a measured 7.2x, and the fold sees one kind of node whatever produced it ([ADR-0066](adr/0066-a-language-is-a-reader-not-a-sense.md)). Swift is the language with no file-grain edge — an import names a whole target — so its answer is given at the target grain and projected onto files, over-reaching in the direction selection is allowed to. `variance reach --since <ref>` hands that answer to a pipe: a run list rather than a skip list, so every path that would print an empty one exits `2` with stdout untouched ([ADR-0067](adr/0067-a-run-list-refuses-where-a-skip-list-degrades.md), [`docs/polyglot.md`](../polyglot.md)). A Gradle, SwiftPM or `pytest` suite gets change-based selection with no framework integration and no recording. |
| B24 | **test-file selection** — instrumentation and selection added to an existing runner configuration or CI job | landed for Vitest. `withTestSelection` owns source instrumentation, per-test-file attribution, journal merge, and the persisted coverage index; `selectTestFiles` maps a unified diff back to code-unit-sorted test-file paths. The first persisted shape was a pretty-printed object graph and was refused immediately: the index now follows `sense`'s measured storage design from journal 0026 — interned strings, aligned typed-array columns, and a CSR block-to-test relation at four bytes per crossing — under the repository-keyed user cache. The selector opens views and returns only final test paths rather than materializing the graph in JavaScript. Conventional Vitest remains the runner and receives whole files only. The external fixture runs its two files as separate shards into one index, then a branch-only diff returns only the file that entered that branch. Storybook remains the exception allowed to select individual stories because the product owns that execution surface. **The Rust question is answered here too, and again the answer is no** ([journal 0041](journal/0041-what-a-rewrite-would-buy.md)). The two largest costs in the record path were a hand-written SHA-256 standing where the platform's compiled one was available, and the parser's own JSON handoff; both came back without native code, and what is left is 38% already native in `instrument` and 40% in `encode`, against a merge and a decode whose remainder is a format that makes the reader pay for everything to reach anything. The one cost a suite pays on every test is the probe, and it runs inside the user's V8 where there is no other side to move it to. **The walk that places the probes is the addon's alone** ([ADR-0071](adr/0071-the-block-walk-is-the-addons.md)): the JavaScript walk is deleted, a golden test holds the identities it produced, and a machine without the addon does not record. **A change is read before it is charged** ([ADR-0072](adr/0072-a-change-is-read-before-it-is-charged.md)): the addon gives each changed file one verdict over both texts, and a moved top-level value is charged to its lexical readers one file deep, so an edit at the top level no longer selects every test that loaded the module (84.8% → 13.9% of changed files over the corpus). |
| B25 | **test-file deviation** — static reach is the denominator and observed source is the slice | landed and measured against the external Vitest fixture. One row belongs to one test file. Its baseline is the non-blank source lines in every repository module reachable along Sense's dependency edges, excluding the test file itself; its slice is the non-blank source lines owned by the innermost observed regions for that file. Sensitivity is slice LOC divided by baseline LOC and deviation is its complement. Suite variation coverage unions files and lines before dividing, so shared modules are never counted twice; suite sensitivity averages the per-file ratios, so a large test file cannot erase a small one. The compact snapshot records passed files even when they crossed no instrumented block and distinguishes synthesized source-less regions, so neither disappears into a false zero or claims a line. The calculation streams counters rather than rebuilding a line-key object graph per test. |
| B26 | **source-to-test lookup** — an agent or editor asks which named tests reached one function, line, or whole source file, and how far away they were | landed as the runner-independent `coveringTests` and `coveringTestsInFile` APIs and the `variance_source_tests` MCP tool. A source line resolves to the innermost indexed execution region, a function name resolves to its function-entry region, and the result contains individual test identities ordered by their shortest observed call-stack depth. The file query partitions every indexed source line into inclusive ranges with one shared answer, including empty answers for indexed regions no test reached while omitting lines the index cannot describe. Stable ids distinguish duplicate names. The MCP surface takes the same `ExecutionIndex` from a collector or editor integration and answers point and file queries without controlling the runner or inventing missing coverage. |
| B27 | **runtime scenarios** — AAA recorded as a state machine over named preconditions | landed as [`@variance-authority/scenario`](../../packages/scenario) ([`docs/scenarios.md`](../scenarios.md), ADR-0047/0048). Arrange holds the planned subject and observed state, authored Act keys label transitions, and Assert separates the initial difference, each transition effect, and cross-execution effect divergence. Executions fold into a partial graph by render hash: convergence shares a node, branching retains every witnessed edge, and failed observation remains unknown. The default is an ephemeral value; the opt-in archive stores canonical semantic snapshots once by content digest under declared retention, access, deletion, and admission policy. It writes no raster, baseline, history row, approval, or exit code. No host adapter records scenarios automatically; the host that owns the arranged world calls the library. |
| B28 | **presentation sensing** — rendered relationships exposed to a coding agent without a design policy or regression lifecycle | landed as [`@variance-authority/presentation`](../../packages/presentation) ([`docs/presentation.md`](../presentation.md), ADR-0050/0051/0052/0053/0054). One live Playwright subject becomes a graph of semantic anchors, geometry, spacing, axes, inferred baselines, surfaces, prominence, repetition, and calibrated relationship findings, with diagnostic paint from the same report. Findings and paint identify the structural owner of their relationship. A pure focus keeps nested boxes separate by default, isolates a stable finding id, and paints without another acquisition; explicit alignment and spacing readings cover product-known flows across wrappers and the rhythm between heterogeneous siblings without declaring all descendants peers. Repeated structures retain the spacing relation from a distinct leading label into their body separately from the relation among body peers; a collision paints both steps. Product-known relationships can instead enter through a typed outside-in hierarchy contract that rejects invalid ownership, order and reused edges: product meaning names the role, while a design-system token remains implementation evidence. Acquisition follows that boundary too: unrelated document images do not hold the subject open and React provenance the report does not consume is not collected. A caller-owned static archive can explicitly replace stabilization with an empty recipe when its page clock cannot advance. Playwright's ARIA snapshot remains an independent retained signal: empty and partial are readings, absence alone is unobserved. Density, height, margins, utilization, and content volume are telemetry and cannot create findings. The package owns no baseline, approval, build verdict, or regression classification; an optional report comparison keeps content identity and information counts beside finding counts, and a product-aware collector can project its introduced, resolved and persisted consequences into the general run report without changing that report's verdict. The packaged [`variance-presentation` skill](../../packages/presentation/skills/variance-presentation/SKILL.md) makes hierarchy mapping the first move before findings or paint. |
| B29 | **update attention** — distinguish the component instance that initiated a React commit from everything React rendered because of it | landed across `react`, Eyes, `distill`, CLI and MCP ([ADR-0055](adr/0055-update-initiators-are-structural-attention.md), [journal 0039](journal/0039-the-updater-had-a-location.md)). The pre-navigation Eyes agent installs the commit tap before application code, copies `memoizedUpdaters` as bounded innermost-first paths of name, key and props digest, and places each commit in the authored AAA chronology. `variance distill` and `variance_distill` share one analyzer that separates initiators structurally inside an addressed target from those outside it, beside whole-test execution crossings rather than collapsing the three. `PerformedWork` remains the rendered-component reading. Public bounded Fiber subtree, parent-chain, composite-path and source-location helpers expose the same diagnostic seam without a page registry or mutation channel. The repo skill turns deterministic opportunities into one-at-a-time counterfactual checks while preserving unavailable versus measured empty. |
...
| M31 | B24 | Build the smallest provided Vitest integration and prove source + tests → coverage data, then coverage data + diff → test-file paths against an external fixture | **expected** — two independent Vitest invocations merge into one persisted index; editing the branch entered only by `alpha.case.ts` selects only that test file. Vitest's setup-file and reporter lifecycle are sufficient: no per-test-case control and no replacement runner. |
| M32 | B25 | Join each covered test file to its forward Sense closure, assign source lines to innermost regions, and calculate baseline, slice, coverage, sensitivity and deviation | **expected** — the external fixture's shared static baseline is 1 file / 9 LOC; `alpha.case.ts` owns 5 LOC, `beta.case.ts` owns 3 LOC, and their variation coverage is 1 file / 6 LOC. Sensitivity is `(5/9 + 3/9) / 2 = 4/9`; the two deviations are `4/9` and `6/9`. A constructed type-only test proves a passed file with no dynamic crossing remains present with a zero slice. Source-less synthesized regions are excluded from line ownership. |
| M33 | B26 | Expose a runner-independent reverse-index query for a source line or function, returning named tests by minimum observed stack depth | **expected** — the constructed nested-function index returns one named test for a branch line while excluding two tests that only entered its enclosing function; the function query returns all three identities at depths 2, 4 and 5. Two observations of one test collapse to depth 2, duplicate names remain distinct by id, and ordering is distance then code unit. Missing source returns no claim and invalid crossing references or distances throw. |
| M34 | B26 | Answer every line of one source file without making an editor issue one point query per line | **expected** — the same nested index becomes three covered ranges around the branch and one unreached range. Adjacent lines with identical test identities and depths share one row, the synthesized source-less branch creates no row of its own, and a file absent from the index returns no claim. |
| M35 | B26 | Put the source-to-test answer on an agent protocol rather than leaving it as a library primitive | **expected** — `variance_source_tests` is listed and called through the MCP protocol over an `ExecutionIndex`; line and function inputs return named tests by depth, file-only input returns compact ranges, an unknown file names the available paths, and ambiguous targets are refused. An unindexed point is distinct from an indexed region no test reached. The subject supplier is read per request, so an integration can expose a rerun without restarting the MCP connection. |
| M36 | B27 | Record named Arrange states and authored Act outcomes, compare transition effects across executions, and retain semantic evidence only when explicitly archived | **expected** — a shared edit can move every state while equal Act effects keep one digest; a handler edit names the first divergent Act. Inserted or repeated Acts stop prefix alignment, an unobserved outcome ends the path, and two destinations under one state and Act remain two machine edges. A second archive instance reopens the path, equal semantic objects occupy one file, expiry is unobserved, and admission refusal writes nothing. |
| M38 | B29 | Put React's update initiators in the same evidence path as authored test attention, then ask whether they occurred inside the component path the test addressed | **expected, after one refused join** — `memoizedUpdaters` supplied the initiators, but display-name equality could not distinguish two live instances. Retaining name, key and props digest for every enclosing composite made the relation structural. The Eyes init script met the commit hook before React loaded, the archive rejected malformed paths at the process boundary, and MCP reported matching `DrawingPanel` work separately from a `Clock` updater outside the addressed path. The existing Vantage/MCP connection carried the answer; no helper process, page registry, active callback or event replay entered the product. |
| M37 | B28 | Sense and paint presentation relationships on one live subject while retaining independent ARIA evidence and refusing density policy | **mixed, and six corrections were architectural** — the original ten pure and two Chromium cases established local relationship sensing, then live use showed that a locator is not a peer set. Owner focus keeps nested evidence out of a composition, explicit flow alignment distinguishes a 2px centre spread on Atlassian navigation from zero spread on GitHub and Paper controls, explicit composition spacing retains relationships between heterogeneous siblings, repeated structures compare leading-label and body-peer relations separately, typed product contracts name outside-in roles that neither inference nor a design-system token may authorize, and presentation consequences now survive as an independent general-report signal rather than being flattened into render impact or verdict. On local `#agents`, seven acquisitions had a 76.63ms median; 1,000 pure owner projections took 8.07ms, and one finding reduced 189 paint instructions to one. A content-preserving five-record edit moved eight findings to zero. The supplied Underwriter archive measured five consecutive major-region gaps at 0–0.01px before a 38.38px transition; its 31 demand rows were separated at 11.18–11.19px, while five inferred patterns report the pictured 4px leading-to-body and 4px body-to-body hierarchy collision. The explicit paired contract reports the same equal middle steps and refuses reversed roles or an owner below the relationship. Its stored effect retains the contract and the 4px/3.99px measurements, while the report file, HTML and agent text read the same value without another browser. |
...
- **The documentation is checked now, and one class of it still is not.**
  ...
  path and every `file:line` reference across all tracked Markdown files, and compiles
...
**B28 is landed as a hierarchy-aware sensing and support surface, not another
regression mode.** Live Atlassian, GitHub and Paper examples now challenge the
owner and cross-wrapper alignment model; the named Underwriter page remains the
...
**Update attention (B29) reaches the agent, but not the scheduling statement.**
The Eyes init script meets React before the application loads and retains the
component instance paths that initiated each commit. Sense retains source
execution for the whole test. Nothing correlates the updater to the exact
callback, timer, store write, or source statement that scheduled it, so the
surface names a structural origin and refuses source causation.
