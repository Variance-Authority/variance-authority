...
Drawn, the wall clock bottoms out at six readers, and what the kernel spends on
one file climbs the whole way:
...
The first row and the third drawn, `open` then `read` in orange and `open`
then `close` in grey:
...
The two wall columns drawn, sense in orange and ripgrep in grey:
